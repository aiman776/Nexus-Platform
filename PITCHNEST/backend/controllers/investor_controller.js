...
const createInvestorProfile = async (req, res) => {
    try {
        const { name, company, location, bio, investmentStage, investmentInterests, minimumInvestment, maximumInvestment, totalInvestments, portfolioCompanies } = req.body;
...
const updateInvestorProfile = async (req, res) => {
    try {
        ...
        if (!updatedInvestor) return res.status(404).json({ message: 'Investor profile nahi mili' });

        res.status(200).json({ success: true, message: 'Investor profile update ho gayi!', investor: updatedInvestor });
...
module.exports = { getAllInvestors, getInvestorById, getMyInvestorProfile, createInvestorProfile, updateInvestorProfile };